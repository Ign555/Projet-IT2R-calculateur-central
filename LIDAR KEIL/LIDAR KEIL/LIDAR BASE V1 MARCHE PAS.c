...
int main (void){
	...
	for (i=6; i<150;i+=5){
		...
		SetAngle = (ValeurRecue[1+i] >> 1) + (short)(ValeurRecue[2+i] << 7);						//on récupère les données de l'angle
		SetAngle_f = (((float)SetAngle) / 64.0);																	//d'après la doc, valeur angle = donnée angle / 64 (en degrés)
		
		sprintf(chain,"angle = %05d",(short)SetAngle_f);
		GLCD_DrawString(0,0,chain);
		
		SetDistance = (ValeurRecue[3+i]) + (short)(ValeurRecue[4+i] << 8);							//on récupère les données de la distance
		SetDistance_f = ((float)SetDistance) / 4.0;															//d'après la doc, valeur distance = donnée distance / 4 (en mm)
		
		sprintf(chain,"distance = %06d",(short)SetDistance);
		GLCD_DrawString(0,30,chain);
		
		Temporisation_ms(300);
